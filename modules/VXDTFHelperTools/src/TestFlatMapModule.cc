...
#include "tracking/modules/VXDTFHelperTools/TestFlatMapModule.h"
...
//                 Register the Module
//-----------------------------------------------------------------
REG_MODULE(TestFlatMap)

//-----------------------------------------------------------------
//                 Implementation
//-----------------------------------------------------------------

TestFlatMapModule::TestFlatMapModule() : Module()
{
...
TestFlatMapModule::~TestFlatMapModule()
{

}


void TestFlatMapModule::initialize()
{
...
void TestFlatMapModule::beginRun()
{
  m_eventCounter = 0;
}


void TestFlatMapModule::event()
{
  m_eventCounter++;
  if (m_eventCounter % 500 == 0) {
    B2INFO("TestFlatMapModule - Event: " << m_eventCounter);
  }
...
void TestFlatMapModule::endRun()
{
}


void TestFlatMapModule::terminate()
{

	B2WARNING("boost::flat_map-Filling took " << (m_eventStuff.count()) << " nanoseconds")
  B2WARNING("boost::flat_map-Reading took " << (m_fillStuff.count() ) << " nanoseconds")
}

void TestFlatMapModule::FillMap() {
	LittleHelper littleHelperBox = LittleHelper();
...
void TestFlatMapModule::FillVector(vector<unsigned int>& vec) {
	...
	B2INFO("TestFlatMapModule::FillVector: vector has " << vec.size() << " entries...")
}

void TestFlatMapModule::JumbleVector(vector<unsigned int>& vec) {
	LittleHelper littleHelperBox = LittleHelper();
